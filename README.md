...
- **Routines** — cloud agents write reports to `reports/`: trades (Mon 6:50am), waivers (Tue 6:50am), lineup (Thu 6:50am), inactives (Sun 10:40am, once the early-slate list is posted), plus a roster watcher that checks twice a day. Each publishes to `main` with `scripts/publish-report.mjs` — anything else strands the report where the dashboard can't see it. Runbook: `docs/ROUTINES.md`.
