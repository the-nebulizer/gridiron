...
| field | type | rule |
|---|---|---|
| `week` | integer | The NFL week the report is about. Required. Sleeper flips its week number on Tuesday, so only `start` actions expire — by week, or by their own kickoff, whichever comes first (see "stale" below); every other kind stays open until it is done or a newer report of the same type replaces it. |
...
|---|---|---|---|
| `add` | `drop` (id, optional), `mode` (`fcfs` or `waiver`), `faab` (integer $, waiver mode only), `displaces` (starter id or `null`, required — see "The case"), `evidence` (object or `null`, required — see "Evidence") | `player` rostered by **no** team in the league; `drop` (if given) on my roster; `faab` between 0 and my `faab_remaining`; `mode: "fcfs"` only while the pool really is first-come-first-serve, which is read off the league's own transaction log rather than the calendar: always true before kickoff, and true in season once a `free_agent` add has **completed this week** (a claim that had to clear waivers is logged as `waiver`, with a bid). No such evidence yet this week and an `fcfs` add is rejected — the safe direction, since "add now, $0" about a player who actually locks until Wednesday is the mistake with a deadline on it. This used to be gated on `games_have_started` alone, which is right only until the week's waiver run happens; afterwards Sleeper hands the pool back out for the rest of the week, and that window is exactly when a streamed kicker or defense gets picked up | `player` on my roster. If another team rostered them first, the card shows "taken by <owner>". |
| `drop` | — | `player` on my roster | `player` no longer on my roster |
| `start` | `slot` (one of the league's non-BN positions), `for` (id of the starter being benched, optional) | `player` on my roster and not in `reserve` (IR has to be activated first); `player` not on a bye this week; `player`'s team, and `for`'s team if given, still `pre_game` in `snapshot.games` (Sleeper locks a player's slot at his own kickoff, so a swap naming either side of a locked slot is rejected); `player`'s position must be legal in `slot`; `for` (if given) currently in my starters **and currently holding that same `slot`** (the swap must be in place, or it empties `for`'s slot); with no `for`, `slot` must already be empty | `player` in my starters and `for` (if given) not in my starters |
...
| `trade` | `with` (roster_id), `give` (ids), `get` (ids), `message` (sendable offer text, optional), `displaces` (starter id or `null` for the first player in `get`, required — see "The case"), `evidence` (object or `null`, required — see "Evidence") | `with` ≠ my roster_id; every `give` on my roster; every `get` on `with`'s roster | every `get` on my roster. The page cannot see pending offers, so trades also get a manual "dismiss" on the card. |
...
| `asks` | Trades only. What does this ask of the other manager? | computed: the partner's own roster — whether they start, bench or reserve each incoming player, their depth at that position, and any injury tag |
...
- **`add`** —
  ...
  `need`: "TE: you have 1 — your thinnest spot · fixes W11", "QB: you have 2 — starts 2, no cover", or "WR: you have 4 — not thin · fixes nothing on the calendar". Three readings, strongest first: thin per `outlook.roster_shape.thin_positions` (can't fill the dedicated slot), uncovered per `no_cover_positions` (losing one body there leaves a slot nobody can fill — asked by re-solving the lineup without one body at that position, so a flex slot a spare receiver could take is not a shortage, while a second quarterback for a QB-held SUPER_FLEX still is), otherwise depth. "fixes W<n>" lists weeks whose `empty_slots` the move removes.
...
- **`trade`** —
  ...
  `asks`: the other side of the deal, read off the partner's roster rather than taken from the report — "Tally241 starts Aaron Jones, Questionable — one of 4 RBs they carry", or "amandaberry benches Brian Robinson — one of 5 RBs they carry". When every player asked for sits in the partner's starting lineup and nothing offered starts in mine, it appends "· asks them to bench a starter for bench pieces", and the compiler prints a warning as well — in both modes, never fatal, because an offer worth sending anyway is the report's call to argue. The line exists because a trades report claimed a player was "still on Tally241's bench" while the snapshot had him in their starting lineup: the compiler held that fact and never looked at it, so the card repeated the claim and the offer sat unanswered for four days.
...
Alongside the lines, every case carries **`tradeoff`** — the same outlook diff the `later` line reads, kept as data rather than prose: `{ net, fixes: [{week, slot, kind}], opens: [...] }`, where `net` is `costs_only` (opens something, closes nothing), `fixes_only`, `mixed`, or `neutral` (changes no week either way). `kind` is `hole` or `downgrade`.

It exists because the lines alone were not enough. On 2026-09-22 the top row of the card was a claim whose own case grid said "fixes nothing on the calendar" and "opens a W11 TE hole" — and the row was drawn in exactly the same weight, colour and position as a move that helps. Two neutral sentences of evidence against a recommendation are easy to read straight past. So the page now marks a live `costs_only` action **costs more than it fixes** on the row itself, and leaves the specifics to the `later` line below it. The flag is computed, never written by a routine, and it is not a veto: a move can be worth making for reasons the calendar can't see, and the report is where that argument goes.

One artifact worth knowing, since it looks like a bug and isn't: fixing a week that previously could not be filled at all can *reveal* a SUPER_FLEX downgrade that was always latent (the downgrade check only runs on weeks that field a legal lineup), so such a move can read as `mixed`. It can never produce a false `costs_only`, because the reveal requires a fix.
...
### Evidence — the claim a move rests on

The snapshot settles who is rostered, and the compiler checks every claim it can derive from it. What it cannot check is the class of claim an `add` or a `trade` usually turns on: a player's **role, health, or return date**. That arrives from web research and lands in the report as prose, where nothing can falsify it.

It went wrong exactly there. A waivers report argued a $20 bid — a fifth of the budget — partly on "Charbonnet's own bye-week return is still weeks off the earliest optimistic date, and he's not close to being activated", when his own coach had him on an aggressive timetable he was on track to meet, targeting Week 5. The player being bought was the backup to the man that return would displace.

So every `add` and `trade` carries `evidence`, and it follows the `displaces` pattern exactly — an explicit answer is required, and one of the answers is "none":

| value | meaning |
|---|---|
| `{ "note": "<one sentence, ≤200 chars>", "url": "<http(s) link>", "as_of": "<YYYY-MM-DD>" }` | the outside claim, and where it came from. `as_of` is optional |
| `null` | the call rests on the roster alone — no outside claim to check |
| key absent | rejected by `--check`; a compile over an older report notes it and sets `evidence_missing` |

`url` must be `http://` or `https://`. The card renders the note as a link, and `reports/actions.json` is re-read from the repo at runtime, so a `javascript:` or `data:` url is a script injection rather than a source — refused at write time, and rendered as plain text rather than a link even if one somehow arrives.

Two things this buys, and only the second is about citation: the card gives Ben a link he can click, so the argument is auditable rather than asserted; and writing the sentence down forces the routine to confirm it read the claim rather than remembered it. `null` is never a workaround — a report whose `why` makes an injury or role claim while `evidence` is `null` is contradicting itself in a way a reader can see.
...
- The same rule on the **acquiring** side: two or more unlinked actions that each bring in a body at one position, where together they would leave the roster two or more past what the lineup starts there. Identical blind spot, identical cause — each action's `need` line reads the baseline as if the others never happened. `if_not` alternatives count once (at the component's largest single arrival), and filling a real hole never trips it, since a position short of its slots ends level rather than deep. Strict under `--check` (the routine writing the report must link them or drop one); a warning at compile time, so one routine's open action can never stop another from publishing. This is the shape the card carried on 2026-09-22: a $20 claim for a fourth running back plus two trade offers each bringing a back home, all three arguing one thin-RB need, while the kicker and defense holes that actually break weeks went unaddressed.
...
| already done / gone / stale | error | carried through with `state` (a stale `start` also carries a `state_reason`), logged, exempt from the strict rules and from sequencing (they hold no player, need no bench slot, spend no FAAB) |
...
- Stamps each source with `written_at`: the report's last git commit time when it is committed and unmodified, otherwise its file mtime (the report being written right now, or a checkout without git). Filenames carry only a date, and the card's "next check" line sorts sources by this rather than by filename so four reports published on one day order by when they were actually written.
- Marks a source `stale: true` when its `week` is behind `snapshot.week` (informational). Drops only `start` actions whose `week` is behind `snapshot.week`; every other kind is kept. Every compiled action carries its `week`. Separately, and even when its source isn't week-stale, an open `start` whose player or `for` has already kicked off (per `snapshot.games`) is kept but compiles with `state: 'stale'` and a `state_reason` explaining which side kicked off, rather than being dropped — the same state a week-behind `start` gets on the card (see "The card" below).
...
```json
{
  ...
  "sources": {
    "waivers":   { "report": "2026-09-15-waivers.md", "week": 2, "verdict": "...", "next_check": "...", "stale": false, "written_at": "2026-09-15T12:01:44.000Z" },
    "lineup":    { "report": "2026-09-10-lineup.md",  "week": 1, "verdict": "...", "next_check": "...", "stale": true,  "written_at": "2026-09-10T12:02:10.000Z" }
  },
  "actions": [
...
- `superseded` — an `if_not` fallback whose parent was done. Shelved with the other closed states below.
- `done` — the live test in the table above passes. Shelved under a "Done · no longer possible · stale · superseded" toggle, together with `gone`, `stale` and `superseded` (below), each counted separately in the toggle's own label.
- `gone` — the move is no longer possible: an `add` whose player is now on someone else's roster (shown with the owner's name), or a `start` / `ir` / `activate` / `drop` whose player has left my roster. Shelved, never shown as an instruction.
