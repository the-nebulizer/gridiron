...
4. Recommend: up to 5 adds ranked, each with a FAAB bid sized against my `faab_remaining` and the standing plan in `docs/SEASON-PLAN.md` (reserve budget per the bye table in `docs/SEASON-PLAN.md` — W13 and W7 are the crunch weeks; W6 is covered — and read every bye from the snapshot, never from memory; QBs are gold in superflex — grab any startable QB who ever hits waivers). Name the drop for each add, from my bench, with one sentence.
