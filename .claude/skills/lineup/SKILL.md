...
5c. **Which defense and kicker start.** If two are rostered, `node scripts/stream.mjs` and `node scripts/stream.mjs --pos K` print who each one plays this week; pick on the matchup and say who they play in one clause. If only one is rostered and it is on bye, the slot scores zero — that is a waiver job, so say so and hand it to `/waivers` rather than leaving the slot to look filled.
