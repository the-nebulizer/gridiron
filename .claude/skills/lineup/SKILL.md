...
5a. **Look past Sunday.** Read the snapshot's `outlook`. Close this week's section by naming the next `crunch_weeks` entry and what it needs — a slot the outlook says I cannot fill (`empty_slots`) is a waiver job, not a lineup job, so hand it to `/waivers` in one sentence rather than solving it here. Never bench a player this week purely to protect a future week; lineups are set weekly.
