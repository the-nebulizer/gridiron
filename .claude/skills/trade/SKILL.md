...
Never: value a QB off a standard-scoring trade chart; propose a player the snapshot shows on a different roster than assumed; ignore what the OTHER manager needs (a trade they won't accept is worth nothing); emit an action for a player the snapshot doesn't place exactly where the action claims; send two offers that give the same player without linking them `if_not`.
