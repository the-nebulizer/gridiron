...
5. For every starter who is Out, Doubtful, or reported inactive, name the bench pivot in **this league's scoring** (superflex, 6-pt pass TDs, half-PPR — a healthy QB in SUPER_FLEX still nearly always outranks a WR/TE pivot). This is a today-only decision: pivot for today's points and never hold a better player back to protect a later week. If no legal pivot exists for a slot, say so plainly and note whether the snapshot's `outlook` already shows that slot empty in weeks ahead — that is a waiver job for `/waivers`, not something to solve on game day.
